...
public:
    ...
    void agregarNodo(structs* nuevo)
    {
        ...
        if(rs != NULL)
        {
            ...
            memcpy(last,rs->lastName,30);
            if(agg(&rs,nuevo) == -1)
            {
                delete[] last;
                return;
            }
            if(strcmp(last,rs->lastName) != 0)
            {
                int pos = getRegistroPosFromFile(*file,rs->lastName);
    ...
    void PreOrder(char lastName[30])
    {
        cout<<"\nDatos en el subArbol[PreOrder]: "<<lastName<<endl;
        imprimirArbolPre(buscarR(lastName,rs));
    }
    void PosOrder()
    {
    ...
    void PosOrder(char lastName[30])
    {
        cout<<"\nDatos en el subArbol[PosOrder]: "<<lastName<<endl;
        imprimirArbolPos(buscarR(lastName,rs));
    }
    void InOrder()
    {
    ...
    void InOrder(char lastName[30])
    {
        cout<<"\nDatos en el subArbol[InOrder]: "<<lastName<<endl;
        imprimirArbolIn(buscarR(lastName,rs));
    }
    ...
    {
        nameArchivo= "C:/Users/ultim/CLionProjects/ED2_Proyecto/cmake-build-debug/datab.dat";
        ...
        if(fi)
        {
            ...
            for(int i = 0; i < cantRegistros; i++)
            {
                ...
                if(buscar(lastName) == -1)
                {
                    ...
                    nuevo->posRegisListaNombre = m->getLastPos();
                    agregarNodo(nuevo);
                }
                else
                {
    ...
    structs* elimi(char last[30], structs** sr)
    {
        ...
        if(strcmp((*sr)->lastName,last) == 0)
        {
            ...
            else
            {
                ...
                if(strcmp(rs->lastName,(*sr)->lastName) == 0)
                {
